...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Action's preconditions must be satisified by the previous state level for it to be reachable.
            if a_node.prenodes.issubset(previous_s_level):
                ...
                for s_node in previous_s_level:
                    # Connect this node to the previous state level and vice-versa
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)
        # Add node to new level of actions
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1's positive effects aren't inconsistent with node_a2's negative effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Ensure node_a2's positive effects aren't inconsistent with node_a1's negative effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1's positive effects don't interfer with node_a2's negative preconditions
        # Ensure node_a2's positive effects don't interfer with node_a1's negative preconditions
        for effect in node_a1.action.effect_add:
        ...
        # Ensure node_a1's negative effects don't interfer with node_a2's positive preconditions
        # Ensure node_a2's negative effects don't interfer with node_a1's positive preconditions
